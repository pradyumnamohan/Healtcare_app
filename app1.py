...
# Move configurations **before** initializing db
...
def send_email(subject, body):
    ...
    from_email = "shreesha447@gmail.com"  # Replace with your email
    to_email = "sanjanar190204@gmail.com"
    password = "rqza thsq wavo uqun"  # Replace with your email password
...
def emergency_redirect(service):
    # Retrieve the latest chat summary
    try:
        with open(FILE_PATH, "r") as file:
            data = json.load(file)
            latest_chat = data[-1] if data else None
    except Exception as e:
        print(f"Error reading chat file: {e}")
        latest_chat = None

    if latest_chat:
        summary = latest_chat.get("summary", "No summary available.")
        subject = f"Emergency Alert: {service.capitalize()}"
        body = f"An emergency has been triggered for {service.capitalize()}.\n\nChat Summary:\n{summary}"
